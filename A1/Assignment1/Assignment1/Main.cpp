...
void ExecuteOption3(const Vector<WeatherRecord> & records, int year)
{
    // Exact Specification Format Match (Option 3): Loops months 1 to 12
    cout << year << endl;
    cout << fixed << setprecision(1);

    for (int m{1}; m <= 12; ++m)
    {
        double totalRadiationWatterson{0.0};
        bool hasData{false};

        for (int i{0}; i < records.Size(); ++i)
        {
            if (records[i].GetDate().GetYear() == year && records[i].GetDate().GetMonth() == m && records[i].IsValidSolar())
            {
                if (records[i].GetSolarRad() >= 100.0)
                {
                    totalRadiationWatterson += records[i].GetSolarRad();
                }
                hasData = true;
            }
        }
        ...
        {
            double totalKwh = (totalRadiationWatterson * (1.0 / 6.0)) / 1000.0;
            cout << totalKwh << " kWh/m2" << endl;
        }
    }
}
...
void ExecuteOption4(const Vector<WeatherRecord> & records, int year)
{
    ...
    for (int m{1}; m <= 12; ++m)
    {
        ...
        if (speedSub.Size() > 0)
            outfile << CalculateAverage(speedSub) << "(" << CalculateStandardDeviation(speedSub) << ")";
        outfile << ",";

        if (tempSub.Size() > 0)
            outfile << CalculateAverage(tempSub) << "(" << CalculateStandardDeviation(tempSub) << ")";
        outfile << ",";
...
void RunMenuLoop(const Vector<WeatherRecord> & records)
{
    ...
    while (choice != 5)
    {
        cout << "\n================ WEATHER MENU :) ================\n"
             ...
             << "3. Get Solar Radiation Energy Accumulations (Yearly Selection)\n"
